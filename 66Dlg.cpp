	...
	: CDialog(CMy66Dlg::IDD, pParent)
{
	...
	m_strRXData = _T("");
	m_strTXData = _T("");
	m_strSavePath = _T("D:\\serial receiver code\\received data\\");//changed by wudan 2014-05-23
	//m_strSavePath = _T("F:\\zy\\data\\");
	m_shortSerialNo = 4;//changed by wudan 2014-05-23
	//m_shortSerialNo = 6;
	m_intEndFlag = 52;//changed by wudan (old one:101)
	//}}AFX_DATA_INIT
...
void CMy66Dlg::OnComm() 
{
	...
	if(m_ctrlComm.GetCommEvent()==2) //value 2 means receive buffer is not empty.
	{
		...
		//changed by wudan 05-24
				if(j==25)//process receive data, when the data match with the EndFlag, add a time tag at the end of data.
				{
					if (m_strRXData.GetLength() >1024*8)
					{
						m_strRXData = "";//received data that displayed in the softeare interface
					}
					/**** added by wudan 05-24 ****/
					rxtemp.Format("%d, ",bt);
					m_strRXData += rxtemp;
					/********/
					m_strRXData += "\r\n";
					GetSystemTime(&t);
					/*** changed by wudan 05-24***/
					strtemp.Format("%d, %d, %d, %d, %d, \n",bt,t.wHour+8,t.wMinute,t.wSecond,t.wMilliseconds);
					//sub.Format("%d, ",bt);
                    //strtemp+=sub;	
				}
				else
	...
	//if(cf.GetPosition() < 1024*16)//get current file pointer
	//	cf.Seek((LONG)0,CFile::current);//move current file pointer by zero unit(keep file pointer unmoved);added by wudan 05-24
	//else{
	//    //cf.SeekToBegin();
	//    //cf.Close();
	//	if(!cf.Open(cf.GetFilePath(),CFile::modeCreate |CFile::modeNoTruncate |CFile::modeReadWrite | CFile::shareDenyNone))
	//	{
	...
	if(current_time - start_time > 60*1) {//maximum time limitation----60 second(by wudan 05-24)
		if (blStarted)
		{
			cf.Close();
...
void CMy66Dlg::OnButtonStart() 
{
	...
	
	{
		...
		if(!cf.Open(m_strSavePath + strtemp + ".csv",CFile::modeCreate |CFile::modeNoTruncate |CFile::modeReadWrite | CFile::shareDenyNone))
		{
			AfxMessageBox(_T("Can not create file : " + m_strSavePath + strtemp + ".csv !!!" ));
		}
